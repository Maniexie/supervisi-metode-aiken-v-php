...
$id_hasil_supervisi = $_GET['id_hasil_supervisi'];
...
$getHasilSupervisi = mysqli_query($koneksi, "SELECT * FROM hasil_supervisi WHERE id_jadwal_supervisi = '$id_jadwal_supervisi' AND id_guru = '$id_guru' ORDER BY id_hasil_supervisi");
$assocHasilSupervisi = mysqli_fetch_assoc($getHasilSupervisi);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (mysqli_num_rows($cekKodeTindakLanjut) == 0) {
        ...
        $stmt = $koneksi->prepare(
            "UPDATE hasil_supervisi SET kode_tindak_lanjut = ? , id_jadwal_supervisi=? , id_guru = ?, nilai = ?, umpan_balik = ?  WHERE id_hasil_supervisi = $id_hasil_supervisi"
        );
        $stmt->bind_param(
            "siids",
            $kode_tindak_lanjut,
...
<section>
    <div class="container">
        <h2 class="text-center">Edit Umpan Balik Hasil Supervisi</h2>
        <form action="" method="post">
            ...
            <div class="mb-3">
                ...
                <select class="form-control" id="kode_tindak_lanjut" name="kode_tindak_lanjut" required autofocus>
                    ...
                    while ($kategoriTindakLanjut = mysqli_fetch_assoc($getKategoriTindakLanjut)) {
                        ...
                        $selected = '';
                        if ($kategoriTindakLanjut['kode_tindak_lanjut'] == $assocHasilSupervisi['kode_tindak_lanjut']) {
                            $selected = 'selected';
                        }

                        echo "<option value='{$kategoriTindakLanjut['kode_tindak_lanjut']}' $selected>
                                    {$kategoriTindakLanjut['kode_tindak_lanjut']} ({$kategoriTindakLanjut['nama_tindak_lanjut']})
                                </option>";
                    }
                    ?>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="umpan_balik" name="umpan_balik"
                    required><?= $assocHasilSupervisi['umpan_balik'] ?></textarea>
            </div>
